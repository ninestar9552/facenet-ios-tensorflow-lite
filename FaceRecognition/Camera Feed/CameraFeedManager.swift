...
class CameraFeedManager: NSObject {
    ...
    init(previewView: PreviewView) {
        ...
        self.previewView.previewLayer.connection?.automaticallyAdjustsVideoMirroring = false
        self.previewView.previewLayer.connection?.isVideoMirrored = true
   ...
   **/
    private func startSession() {
        ...
        // ✅ session이 실행된 후에 connection이 생성되므로, 여기서 설정해야 함
   ...
   */
    ...
    private func addObservers() {
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(
                CameraFeedManager.sessionRuntimeErrorOccurred(notification:)),
            name: NSNotification.Name.AVCaptureSessionRuntimeError,
            object: session)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(
                CameraFeedManager.sessionWasInterrupted(notification:)),
    ...
    private func removeObservers() {
        NotificationCenter.default.removeObserver(
            ...
            object: session)
        NotificationCenter.default.removeObserver(
            self, name: NSNotification.Name.AVCaptureSessionWasInterrupted,
            object: session)
        NotificationCenter.default.removeObserver(
    ...
    @objc func sessionWasInterrupted(notification: Notification) {
        ...
        if let userInfoValue = notification.userInfo?[
            AVCaptureSessionInterruptionReasonKey] as AnyObject?,
